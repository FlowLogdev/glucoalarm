...
export interface QueryIntent {
  ...
  metric: "summary" | "a1c" | "time_in_range";
}

const VALID_RANGES = new Set([1, 7, 14, 30, 90]);
const VALID_METRICS = new Set(["summary", "a1c", "time_in_range"]);
const DEFAULT_INTENT: QueryIntent = { range_days: 1, metric: "summary" };

const SYSTEM_PROMPT = `Parse a WhatsApp message asking for a glucose summary into JSON only, no other text: {"range_days": 1|7|14|30|90, "metric": "summary"|"a1c"|"time_in_range"}.

range_days: map "today"/"now"/no timeframe mentioned -> 1. "week"/"7 days" -> 7. "two weeks" -> 14. "month"/"30 days" -> 30. "3 months"/"quarter"/"90 days" -> 90. Round anything else to the nearest of these five values.
metric: "a1c"/"A1C"/"GMI" mentioned and nothing else -> "a1c". "time in range"/"TIR"/"range" mentioned and nothing else -> "time_in_range". Otherwise, or if unclear -> "summary".
